...
import com.herokuapp.bookmemo.service.UserDetailsServiceImpl;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    http.authorizeRequests().antMatchers("/", "/signup", "/signupsuccess", "/thanks").permitAll()
        .antMatchers("/admin**/**").hasRole("ADMIN").antMatchers("/memo**/**")
    ...
    http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.IF_REQUIRED)
        .sessionFixation().changeSessionId().maximumSessions(1).maxSessionsPreventsLogin(false);
