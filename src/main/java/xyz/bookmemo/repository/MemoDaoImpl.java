...
/**
 * "This class is used to create queries that are used to search for memos."
 *
 * <p>The first thing to notice is that the class is annotated with `@Repository`. This is because
...
public class MemoDaoImpl implements MemoDao {
   ...
   *
   * <p>The first thing we do is create a CriteriaBuilder. This is the object that we use to build
   * our query
   *
   ...
   *
   * <p>The first thing we do is create a CriteriaBuilder object. This is the object that we use to
   * build our query
   *
   * @param account The account to search for.
   ...
   * associated with the given Account."
   *
   * <p>The first thing to notice is that the function is annotated with the @Override annotation.
   * This is because the function is overriding a function in the MemoRepository interface
   *
  ...
  public Long countBookNameByContainingBookNameAndAccount(String bookName, Account account) {
    ...
    query
        ...
        .where(
            builder.equal(root.get(Memo_.account).get(Account_.id), account.getId()),
            builder.like(
                builder.upper(root.get(Memo_.bookName)), "%" + bookName.toUpperCase() + "%"));
  ...
  /**
   * "Get the memos of the account, ordered by the updated date and memo id, and return the memos
   * from the page to the limit."
   *
   * <p>The first thing to do is to create a CriteriaBuilder. This is used to create a CriteriaQuery
   *
   * @param account The account to search for.
   ...
   * ID."
   *
   * <p>The first thing to note is that the function is annotated with `@Override`. This is because
   * the function is overriding a function in the interface
   *
  ...
  public List<Memo> searchTitle(String selectTitle, Account account, int page, int limit) {
    ...
    query
        ...
        .where(
            builder.equal(root.get(Memo_.account).get(Account_.id), account.getId()),
            builder.like(
                builder.upper(root.get(Memo_.title)), "%" + selectTitle.toUpperCase() + "%"))
        .orderBy(builder.desc(root.get(Memo_.updatedDate)), builder.desc(root.get(Memo_.memoId)));
    final TypedQuery<Memo> typedQuery = entityManager.createQuery(query);
    final List<Memo> memos = typedQuery.setFirstResult(page).setMaxResults(limit).getResultList();
    return memos;
  ...
  public List<Memo> searchBookName(String selectBookName, Account account, int page, int limit) {
    ...
    query
        ...
        .where(
            builder.equal(root.get(Memo_.account).get(Account_.id), account.getId()),
            builder.like(
                builder.upper(root.get(Memo_.bookName)), "%" + selectBookName.toUpperCase() + "%"))
        .orderBy(builder.desc(root.get(Memo_.updatedDate)), builder.desc(root.get(Memo_.memoId)));
